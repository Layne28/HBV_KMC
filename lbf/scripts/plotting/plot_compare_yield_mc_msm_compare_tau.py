...
for i in range(0*nseeds):
    ...
    if os.path.exists(myfile):
        ...
        if os.path.getsize(myfile) > 0:
            print(myfile)
            data = np.genfromtxt(myfile, dtype=float, delimiter=',', names=True, usecols=['sweep','NE','NCD_T4'])

            #Get index where capsid first appears
            mask = (data['NE'] == 120) & (data['NCD_T4']== 60)
            print(data['NE'])
            if mask.size!=0:
...
#Plot time evolution of trimers and capsids
    ...
    plt.plot(np.array(range(T+1))*(tau/tau0),p[:,MM_dict['ndimer=120_nCD=60']],color=colors[cnt],label=r'MSM, $\tau/\tau_0=%.0f$' % (tau/tau0))
...
plt.savefig('plots/T4_std_yield_vs_time_mc_vs_msm_vary_tau.png')
plt.show()
