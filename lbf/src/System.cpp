...
void System::initialize(int Ntype0)
{
...
void System::update_neigh_vertex(int vid0)
{
...
void System::update_neigh_vertex_and_neigh(int vid0)
{
...
int System::is_vboundary(int vid)
{ // 1 if
...
int System::is_bond_vboundary(int vid)
{
...
int System::is_boundary(int heid0)
...
int System::is_bond_in_boundary(int heid0)
{ // it is on the boundary if it has no next prevoius
	if (heid0 == -1)
	{
...
int System::is_bond_out_boundary(int heid0)
{ // it is on the boundary if it has no next prevoius
	if (heid0 == -1)
	{
...
int System::no_bond_boundary(int heid0)
{ // it is on the boundary if it has no next prevoius
	int heindex0 = heidtoindex[heid0];
	//int opindex0=heidtoindex[he[heindex0].opid);
	if ((he[heindex0].nextid == -1 && he[heindex0].previd == -1))
...
int System::is_same_triangle(int heid0, int heid1)
{
	if (heid0 == heid1)
		return 1;
	int heindex0 = heidtoindex[heid0];
...
int System::not_cross_edge(int heid0, int heid1)
{
	int heindex0 = heidtoindex[heid0];
...
int System::next_open_wedge(int heid0)
{
	...
	{
		cout << "Wrong System vin vout in next_open wedge " << endl;
		exit(-1);
	}
...
int System::pre_open_wedge(int heid0)
{
	if (Nhe < 10)
	{
		return -1;
...
int System::open_wedge(int heid0, int *flag) // ised in add_monomer_dimer
{
	//if (Nhe<21) { return -1;}
...
int System::next_connected_boundary(int vid0, int vid1)
{
	/*for (vector<VTX>::iterator it = v.begin() ; it != v.end(); ++it) {
...
int System::connected(int vid0, int vid1)
{
	for (vector<HE>::iterator it = he.begin(); it != he.end(); ++it)
	{
...
int System::connectedH(int heid0, int heid1)
{
...
void System::add_vertex(double *xyz)
{
	VTX *vtxi;
	vtxi = new VTX;
	vtxi->vid = Nvlast;
...
int System::delete_vertex(int vid0)
{
...
int System::remove_neigh(int vid0, int removevid)
{
	int vindex0 = vidtoindex[vid0];
	int counter = -1;
	if (v[vindex0].vneigh.size() == 0)
...
int System::delete_edge(int heid0)
{
...
void System::add_half_edge_type(int vin0, int vout0, int etype, int b_index)
{
...
int System::add_edge_type(int vin0, int vout0, int etype)
{
	int b_index = -1;
	if (vin0 >= Nvlast || vout0 >= Nvlast)
	{
...
int System::opposite_edge(int heid0)
{
	int heindex0 = heidtoindex[heid0];
	...
	cout << " No opposite  Invalid System" << endl;
	exit(-1);
}
...
void System::set_prev_next_boundary(int previd0, int nextid0)
{
...
int System::get_prev_next(int heid0, int *previd0, int *nextid0)
{
...
double System::add_dimer(int heid0, gsl_rng *r, int et1, int et2 , double *distance_vector)
...
int System::force_add_dimer(int heid0, double *newv, int typenext, int typeprev)
{ // adds the dimer if it  has no next prev
	if (heid0 >= Nhelast)
	{
		cout << " in add_dimer ERROR\n";
...
int System::remove_dimer(int heindex0, int heindexnext0)
{
...
void System::he_initialize(int heindex, int heid0, int vin0, int vout0, int etype, int b_index)
{
	he[heindex].id = heid0;
	he[heindex].opid = -1;
	he[heindex].vin = vin0;
...
void System::update_edge(int heid0)
{ //hevec hecent l n
	int heindex = heidtoindex[heid0];
...
void System::update_half_edge(int heid0)
{ //hevec hecent l n
	if (heid0 == -1)
	{
...
int System::add_monomer(int nextofnewid, int prevofnewid, int et)
{ //nextofnew prevofnew
...
int System::force_add_monomer(int nextofnewid, int prevofnewid, int etype)
{ //nextofnew prevofnew
	int heid0 = prevofnewid;
	//cout << "in add_monomer" << "heid0 is prevofnewid " << heid0 <<endl;
	int heid1 = nextofnewid;
...
int System::add_monomer_dimer(int heid0)
{
...
int System::remove_monomer_dimer(int heid0, gsl_rng *r) /* NOT USED */
{
	/*if (!(is_boundary(heid0) > 0))
	{
...
void System::move_v_epsilon(double len_move, double *pi, double *pf, gsl_rng *r)
{
	double *tempvec, *fvec;
...
double System::move_p(double *pi, double *pf, gsl_rng *r)
{
...
double System::find_project_dist_axes(int heindex0, double *newv, double *oldv , double *dis_vector_project )
{
...
double System::move_p_rotate_axes(int heindex0,double len_v, double *pi, double *pf, gsl_rng *r, double *dis_vector)
{
...
double System::move_p_gaussian_axes(int heindex0,double len_v, double *pi, double *pf, gsl_rng *r, double *dis_vector)
{
...
double System::new_vertex_edge_and_move(int heindex0, double *newv, int etnew,gsl_rng *r)
{
...
void System::new_vertex(int heindex0, double *newv)
{
...
void System::new_vertex_points(int heindex0, int ind ,double *newv)
{
...
void System::hecenter(int heindex, double *vcenter)
{
...
void System::helen(int heindex, double *helen)
{
	int vindexi = vidtoindex[he[heindex].vin];
	//cout << " he[heindex].vout " << he[heindex].vout;
	int vindexj = vidtoindex[he[heindex].vout];
...
int System::get_normal(int heid0)
{
	int heindex0 = heidtoindex[heid0];
...
void System::update_excluder_top_he(int heid)
{
...
void System::update_excluder_top()
{
...
void System::update_normals_vertex(int vindex0)
{
...
void System::update_excluder_top_vertex(int vindex0)
{
...
int System::check_inside_overlap(int heid0)
{
	if (Nhe <= 180)
		return 1;
	int heindex0 = heidtoindex[heid0];
	...
	for (vector<HE>::iterator it = he.begin(); it != he.end(); it++)
	{
		...
		if ((it->id != heid0) && (it->opid != heid0))
		{
			...
			if (vi1 != vj1 && vi1 != vj2 && vi2 != vj1 && vi2 != vj2 && vi3 != vj1 && vi3 != vj2)
			{
				...
				if (tempnextid == -1)
				{
					cout << " wrong System! inside overlap" << endl;
				}
				if (tri_tri_overlap_test_3d(v[vidtoindex[vi1]].co, v[vidtoindex[vi2]].co, v[vidtoindex[vi3]].co,
...
int System::do_intersect(int heid1, int heid2)
{
...
int System::find_overlap_g(int vid0)
{
	if (Nhe < 22)
	{
		return 1;
...
int System::check_overlap_g(int vid0)
{
	if (Nhe < 22)
	{
		return 1;
...
int System::check_overlap_centerv(double *newv) /* checks this v point with all other hecenters and vertices*/
{
	if (Nhe <= 6)
		return 1;
...
int System::check_overlap_centerh(double *newcenter) /* checks this he center point with all other hecenters*/
{
	if (Nhe <= 6)
		return 1;
...
double System::find_gbb(int etypenew, int etypenextofnew, int etypeprevifnew)
{
...
double System::find_dg(int type, int typenext, bool drug)
{
...
double System::bend_energy(int heindex0)
{
...
double System::compute_bind_energy()
{
	double tot_bind = 0;
	for (vector<HE>::iterator it = he.begin(); it != he.end(); ++it)
	{
...
double System::monomer_energy(int heid0)
{
...
double System::dimer_energy(int heid0, int heid1)
{
...
double System::vertex_energy(int vid0)
{
	double v_eng = 0;
	int vindex0 = vidtoindex[vid0];
...
/*void System::get_prev_fusion_heid(int heidsurf0){ //not used
	//prev_fusion_heid
...
void System::get_next_fusion_heid(int heidsurf0)
{
...
/*int System::get_fusion_vid(int vidsurf0){
...
void System::update_fusion_pairs_he()
{
...
/*
void System::update_fusion_pairs(){
...
void System::save_vtx(int vid0, VTX *tempvtx)
{
...
/* this fuction should be updated with boundary index */
int read_restart_lammps_data_traj(System &g, FILE *trajfile, int step = -1)
{
...
//void show_status(System &g, int frame, int sweep, int seconds ){
